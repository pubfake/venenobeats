...
module.exports = class PlayCommand extends Command {
  ...
  async run(message, { query }) {
    ...
    if (!voiceChannel) return message.say('<a:roxo_14:727327715568386068> **Você não está em nenhum canal de voz.**');
    // end initial check
      ...
      .then(connection => {
        const dispatcher = connection
          ...
          .on('start', () => {
            ...
            if (queue[1]) videoEmbed.addField('**Próxima Música:**', queue[1].title);
          ...
          .on('error', e => {
            message.say('**Não consigo tocar essa música**');
            console.error(e);
